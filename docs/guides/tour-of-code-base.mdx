...
- `yarn verify` for fast verification (linting, testing, etc.)
- `yarn lint`
- `yarn test` will run all unit tests and e2e [Cypress](https://cypress.io) tests
- `yarn test:specs` will run all unit tests
- `yarn test:e2e` will run all e2e [Cypress](https://cypress.io) tests
...
- The package name (directory) is camel case with hyphens (reflecting package name)
- includes LICENSE and `.babelrc` files
